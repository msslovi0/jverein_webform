...
<link href="styles/MonthPicker.css" media="all" rel="stylesheet" type="text/css" />
<link href="styles/jquery-ui.min.css" media="all" rel="stylesheet" type="text/css" />

<script src="https://{$smarty.server.HTTP_HOST}/scripts/jquery-2.1.4.min.js" type="text/javascript"></script>
<script src="https://{$smarty.server.HTTP_HOST}/scripts/jquery-ui.min.js" type="text/javascript"></script>
<script src="https://{$smarty.server.HTTP_HOST}/scripts/MonthPicker.min.js" type="text/javascript"></script>

{literal}<script>
$(document).ready(function() {
	var DatumAktuell = new Date();
	var JahrAktuell = DatumAktuell.getFullYear();
	$('#monthpicker').MonthPicker({ StartYear: JahrAktuell, ShowIcon: true });
});
</script>{/literal}
...
<p><label for="monthpicker" style="display: inline-block; width: 160px; {validate id='entry_date' message="background-color : #FF0000 ;"}">Eintrittsmonat:</label><input name="{$fields['entry_date']}" id="monthpicker" type="text" size="10" maxlength="10" accept-charset="UTF-8" value="{if isset($form)}{$form.entry_date}{else}{"next month"|date_format:"%m/%Y"}{/if}"></p>
...
<p>Im ersten Jahr wird der Beitrag anteilig zum Eintrittsdatum abgebucht.</p>
</fieldset></div>
</p>
